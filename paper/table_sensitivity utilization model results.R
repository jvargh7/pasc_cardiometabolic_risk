...
predictions <- bind_rows(read_csv(file="sensitivity utilization/pcrsu401_marginal predictions at t by COHORT.csv"),
                         read_csv(file="sensitivity utilization/pcrsu402_marginal predictions at t by COHORT and modifier.csv")) %>% 
  rename(t = x) %>% 
...
difference100 <- read_csv("sensitivity utilization/pcrsu404_difference between 0 and 100 within sociodemographic.csv") %>% 
...
did100 <- read_csv("sensitivity utilization/pcrsu405_difference relative to exposed for difference between 0 and 100.csv") %>% 
  dplyr::filter(modifier2_value %in% c(100)) %>% 
  rename(t = modifier2_value) %>% 
  mutate(group = str_replace(exposure,"COHORT",""),
         facet = str_replace(modifier1,modifier_var,"")) %>% 
...
did0 <- read_csv("sensitivity utilization/pcrsu406_difference relative to exposed for time 0.csv") %>% 
...
bind_rows(predictions ,
  ...
  arrange(facet) %>% 
  write_csv(.,file="paper/table_sensitivity utilization modeling results.csv")
