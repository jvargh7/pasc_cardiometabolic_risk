...
predictions <- bind_rows(read_csv(file="sensitivity encounters/pcrse401_marginal predictions at t by COHORT.csv"),
                         read_csv(file="sensitivity encounters/pcrse402_marginal predictions at t by COHORT and modifier.csv")) %>% 
  rename(t = x) %>% 
...
difference100 <- read_csv("sensitivity encounters/pcrse404_difference between 0 and 100 within sociodemographic.csv") %>% 
...
did100 <- read_csv("sensitivity encounters/pcrse405_difference relative to exposed for difference between 0 and 100.csv") %>% 
  dplyr::filter(modifier2_value %in% c(100)) %>% 
  rename(t = modifier2_value) %>% 
  mutate(group = str_replace(exposure,"COHORT",""),
         facet = str_replace(modifier1,modifier_var,"")) %>% 
...
did0 <- read_csv("sensitivity encounters/pcrse406_difference relative to exposed for time 0.csv") %>% 
...
bind_rows(predictions ,
  ...
  arrange(facet) %>% 
  write_csv(.,file="paper/table_sensitivity encounters modeling results.csv")
