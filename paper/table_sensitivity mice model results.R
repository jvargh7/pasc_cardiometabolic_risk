...
predictions <- bind_rows(read_csv(file="sensitivity mice/pcrsm401_marginal predictions at t by COHORT.csv"),
                         read_csv(file="sensitivity mice/pcrsm402_marginal predictions at t by COHORT and modifier.csv")) %>% 
  rename(t = x) %>% 
  ...
  group_by(t,group,outcome,modifier,facet) %>% 
  summarize(predicted = mean(predicted))

difference100 <- read_csv("sensitivity mice/pcrsm404_difference between 0 and 100 within sociodemographic.csv") %>% 
  ...
  mutate(facet = factor(facet,levels=c("Overall",
         ...
         group = factor(group,levels=c("historical","unexposed","exposed"),
                        labels=c("Historical","Unexposed","Exposed"))) %>% 
  # Edited to variables from clean_mi_contrasts() --------
  rename(predicted = theta_D,
         conf.low = L,
         conf.high = U)


did100 <- read_csv("sensitivity mice/pcrsm405_difference relative to exposed for difference between 0 and 100.csv") %>% 
  dplyr::filter(modifier2_value %in% c(100)) %>% 
  rename(t = modifier2_value) %>% 
  mutate(group = str_replace(exposure,"COHORT",""),
         facet = str_replace(modifier1,modifier_var,"")) %>% 
...
did0 <- read_csv("sensitivity mice/pcrsm406_difference relative to exposed for time 0.csv") %>% 
...
bind_rows(predictions ,
  ...
  arrange(facet) %>% 
  mutate(across(one_of("Exposed","Unexposed","Historical"),
                .fns = function(x) str_replace_all(x,"\\s\\(NA\\,\\sNA\\)",""))) %>% 
  write_csv(.,file="paper/table_sensitivity mice modeling results.csv")
