...
smoking_status <- readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/vital.RDS")) %>% 
...
(prior_clinical <- readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/index date.RDS")) %>% 
  ...
  left_join(lab_history_wide,
            by = "ID")) %>% 
  saveRDS(.,paste0(path_pasc_cmr_folder,"/working/cleaned/lookback clinical characteristics.RDS"))
