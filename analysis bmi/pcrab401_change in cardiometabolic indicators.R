...
save_geeglm(bmi_fit) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab401 bmi_fit.RDS"))
...
bind_rows(
  ...
  write_csv(.,"analysis bmi/pcrab401_coefficients change in bmi.csv")
...
bind_rows(
  ...
  write_csv(.,file="analysis bmi/pcrab401_marginal predictions at t by COHORT.csv")
...
pcra301_contrast_fit <- function(fit,x,y){
  ...
  bind_rows(bmi_contrast %>% mutate(outcome = "bmi")) %>% 
    write_csv(.,"analysis bmi/pcrab401_contrasts of change in bmi.csv")
