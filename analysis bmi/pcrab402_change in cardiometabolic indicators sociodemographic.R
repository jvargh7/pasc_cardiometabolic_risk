...
save_geeglm(bmi_fit_sex) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab402 bmi_fit_sex.RDS"))

save_geeglm(bmi_fit_age) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab402 bmi_fit_age.RDS"))

save_geeglm(bmi_fit_raceeth) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab402 bmi_fit_raceeth.RDS"))

save_geeglm(bmi_fit_hospitalization) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab402 bmi_fit_hospitalization.RDS"))


# Coefficients -----

bind_rows(
  broom::tidy(bmi_fit_sex) %>% mutate(model = "PCRAB402",outcome = "BMI",modifier = "sex"),

  broom::tidy(bmi_fit_age) %>% mutate(model = "PCRAB402",outcome = "BMI",modifier = "age"),

  broom::tidy(bmi_fit_raceeth) %>% mutate(model = "PCRAB402",outcome = "BMI",modifier = "raceeth"),
  
  broom::tidy(bmi_fit_hospitalization) %>% mutate(model = "PCRAB402",outcome = "BMI",modifier = "hospitalization")
) %>% 
  write_csv(.,"analysis bmi/pcrab402_coefficients cardiometabolic indicators sociodemographic.csv")
...
bind_rows(
  ...
  write_csv(.,file="analysis bmi/pcrab402_marginal predictions at t by COHORT and modifier.csv")
