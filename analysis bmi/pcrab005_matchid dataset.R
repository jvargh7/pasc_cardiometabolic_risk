...
matchid_df = bind_rows(
    readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/lookback dataset for analysis.RDS")) %>% 
      dplyr::filter(COHORT == "historical") %>% 
      dplyr::select(ID,COHORT) %>% 
      left_join(readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/lookback dataset for analysis.RDS")) %>% 
