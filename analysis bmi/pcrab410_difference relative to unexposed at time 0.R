...
z = 0
pcrab410_contrast_fit <- function(gee_saved_fit,x,y = NULL,z){
  # x --> exposure
  # y --> modifier1
  # z --> modifier2_value
...
contrast_overall <- map_dfr(1:nrow(difference_grid_overall),
                            function(i){
                              ...
                              bind_rows(
                                pcrab410_contrast_fit(bmi_fit_overall,x=x_name,z=z_value) %>% 
...
contrast_sex <- map_dfr(1:nrow(difference_grid_sex),
                        function(i){
                          ...
                          bind_rows(
                            pcrab410_contrast_fit(bmi_fit_sex,x_name,y_name,z_value) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     modifier2 = "t",
                                     exposure_value = 1,
...
contrast_age <- map_dfr(1:nrow(difference_grid_age),
                        function(i){
                          ...
                          bind_rows(
                            pcrab410_contrast_fit(bmi_fit_age,x_name,y_name,z_value) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     modifier2 = "t",
                                     exposure_value = 1,
...
contrast_raceeth <- map_dfr(1:nrow(difference_grid_raceeth),
                            function(i){
                              ...
                              bind_rows(
                                pcrab410_contrast_fit(bmi_fit_raceeth,x_name,y_name,z_value) %>% 
                                  mutate(exposure = x_name,
                                         modifier1 = y_name,
                                         modifier2 = "t",
                                         exposure_value = 1,
...
contrast_hospitalization <- map_dfr(1:nrow(difference_grid_hospitalization),
                                    function(i){
                                      ...
                                      bind_rows(
                                        pcrab410_contrast_fit(bmi_fit_hospitalization,x_name,y_name,z_value) %>% 
                                          mutate(exposure = x_name,
                                                 modifier1 = y_name,
                                                 modifier2 = "t",
                                                 exposure_value = 1,
...
bind_rows(contrast_overall,
  ...
  write_csv(.,"analysis bmi/pcrab410_difference relative to unexposed for time 0.csv")
