...
save_geeglm(bmi_fit) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity encounters/pcrse401 bmi_fit.RDS"))


# Coefficients -----

bind_rows(
  broom::tidy(bmi_fit) %>% mutate(model = "PCRSE401",outcome = "BMI")
) %>% 
  write_csv(.,"sensitivity encounters/pcrse401_coefficients change in bmi.csv")
...
bind_rows(
  ...
  write_csv(.,file="sensitivity encounters/pcrse401_marginal predictions at t by COHORT.csv")
...
pcra401_contrast_fit <- function(fit,x,y){
  ...
  bind_rows(bmi_contrast %>% mutate(outcome = "bmi")) %>% 
    write_csv(.,"sensitivity encounters/pcrse401_contrasts of change in bmi.csv")
