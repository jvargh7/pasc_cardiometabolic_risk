...
encounter_between_followup_bmi <- readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/pcrpre501_encounter between bmi followup.RDS"))
...
bmi_df = readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/pcrpre401_anthro followup.RDS")) %>% 
  ...
  left_join(encounter_between_followup_bmi %>% 
              ungroup(),
            by = c("ID","MEASURE_DATE")) %>% 
  mutate(across(one_of(encounter_types),.fns = function(x) case_when(is.na(x) ~ 0,
                                                   TRUE ~ x))) %>% 
