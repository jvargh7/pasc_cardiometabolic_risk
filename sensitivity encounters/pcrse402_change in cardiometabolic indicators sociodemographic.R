...
source("sensitivity encounters/pcrse302_analytic dataset with ip weights for bmi.R")

library(geepack)

bmi_fit_sex <- geeglm(formula = as.formula(paste0("bmi ~ COHORT*sex_category*t + lb_bmi +",
                                                  paste0(imbalanced_variables,collapse=" + "),
                                                  "+ ",
                                                  paste0(encounter_types,collapse = " + "))),
                  data = bmi_df,weights=w_sex,id = ID,corstr="exchangeable")
bmi_fit_age <- geeglm(formula = as.formula(paste0("bmi ~ COHORT*age_category*t + lb_bmi +",
                                                  paste0(imbalanced_variables,collapse=" + "),
                                                  "+ ",
                                                  paste0(encounter_types,collapse = " + ")),
                                           ),
...
save_geeglm(bmi_fit_sex) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity encounters/pcrse402 bmi_fit_sex.RDS"))

save_geeglm(bmi_fit_age) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity encounters/pcrse402 bmi_fit_age.RDS"))

save_geeglm(bmi_fit_raceeth) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity encounters/pcrse402 bmi_fit_raceeth.RDS"))

save_geeglm(bmi_fit_hospitalization) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity encounters/pcrse402 bmi_fit_hospitalization.RDS"))


# Coefficients -----

bind_rows(
  broom::tidy(bmi_fit_sex) %>% mutate(model = "PCRSE402",outcome = "BMI",modifier = "sex"),

  broom::tidy(bmi_fit_age) %>% mutate(model = "PCRSE402",outcome = "BMI",modifier = "age"),

  broom::tidy(bmi_fit_raceeth) %>% mutate(model = "PCRSE402",outcome = "BMI",modifier = "raceeth"),
  
  broom::tidy(bmi_fit_hospitalization) %>% mutate(model = "PCRSE402",outcome = "BMI",modifier = "hospitalization")
) %>% 
  write_csv(.,"sensitivity encounters/pcrse402_coefficients cardiometabolic indicators sociodemographic.csv")
...
bind_rows(
  ...
  write_csv(.,file="sensitivity encounters/pcrse402_marginal predictions at t by COHORT and modifier.csv")
