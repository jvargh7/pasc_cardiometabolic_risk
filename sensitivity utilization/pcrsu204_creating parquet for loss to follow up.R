...
source("analysis bmi/pcrsu003_analytic dataset for data availability.R")
...
lookback_processed <- readRDS(paste0(path_pasc_cmr_folder,"/working/models pcrab/pcrab002_imputed lookback dataset.RDS")) %>% 
  # analytic_sample is coming from pcrsu003_analytic dataset for data availability.R
  # We used right_join() instead of left_join() here
...
outcome_df <- lookback_processed %>% 
  ...
  left_join(hd_dataset_censoring %>% 
            ...
            by = "ID")
write_parquet(outcome_df,paste0(path_pasc_cmr_folder,"/working/sensitivity utilization/pcrsu204_ipw for loss to followup data.parquet"))
