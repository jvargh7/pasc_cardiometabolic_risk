...
# imbalanced_variables come from pcrsu302_analytic dataset with ip weights.R 
...
save_geeglm(bmi_fit) %>% 
  saveRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity utilization/pcrsu401 bmi_fit.RDS"))


# Coefficients -----

bind_rows(
  broom::tidy(bmi_fit) %>% mutate(model = "PCRSU401",outcome = "BMI")
) %>% 
  write_csv(.,"sensitivity utilization/pcrsu401_coefficients change in bmi.csv")
...
bind_rows(
  ...
  write_csv(.,file="sensitivity utilization/pcrsu401_marginal predictions at t by COHORT.csv")
...
pcra401_contrast_fit <- function(fit,x,y){
  ...
  bind_rows(bmi_contrast %>% mutate(outcome = "bmi")) %>% 
    write_csv(.,"sensitivity utilization/pcrsu401_contrasts of change in bmi.csv")
