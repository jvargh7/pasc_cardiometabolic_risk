...
mo_weights_df <- readRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity utilization/pcrsu301_ip weights for missing outcomes.RDS"))
# 
...
anthro_followup_summary <- readRDS(paste0(path_pasc_cmr_folder,"/working/sensitivity utilization/pcrsu003_anthro followup summary.RDS"))
...
bmi_df = readRDS(paste0(path_pasc_cmr_folder,"/working/cleaned/pcrpre401_anthro followup.RDS")) %>% 
  ...
  dplyr::filter(ID %in% anthro_followup_summary$ID) %>% 
