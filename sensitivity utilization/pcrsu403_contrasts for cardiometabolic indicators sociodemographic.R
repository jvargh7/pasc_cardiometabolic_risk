...
pcrab403_contrast_fit <- function(gee_saved_fit,x,y,z){
  # x --> exposure
  # y --> modifier1
  # z --> modifier2_value
...
contrast_sex <- map_dfr(1:nrow(difference_grid_sex),
                        function(i){
                          ...
                          bind_rows(
                            pcrab403_contrast_fit(bmi_fit_sex,x_name,y_name,z_value) %>% 
...
contrast_age <- map_dfr(1:nrow(difference_grid_age),
                        function(i){
                          ...
                          bind_rows(
                            pcrab403_contrast_fit(bmi_fit_age,x_name,y_name,z_value) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     modifier2 = "t",
                                     exposure_value = 1,
...
contrast_raceeth <- map_dfr(1:nrow(difference_grid_raceeth),
                        function(i){
                          ...
                          bind_rows(
                            pcrab403_contrast_fit(bmi_fit_raceeth,x_name,y_name,z_value) %>% 
                              mutate(exposure = x_name,
                                     modifier1 = y_name,
                                     modifier2 = "t",
                                     exposure_value = 1,
...
contrast_hospitalization <- map_dfr(1:nrow(difference_grid_hospitalization),
                            function(i){
                              ...
                              bind_rows(
                                pcrab403_contrast_fit(bmi_fit_hospitalization,x_name,y_name,z_value) %>% 
                                  mutate(exposure = x_name,
                                         modifier1 = y_name,
                                         modifier2 = "t",
                                         exposure_value = 1,
...
bind_rows(contrast_sex %>% mutate(modifier_var = "sex_category"),
  ...
  write_csv(.,"sensitivity utilization/pcrsu403_contrasts of change in cardiometabolic indicators sociodemographic.csv")
