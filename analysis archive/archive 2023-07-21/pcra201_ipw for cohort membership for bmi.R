...
bmi_split = initial_split(bmi_df,prop=0.7,strata = COHORT)
...
best_tune_grid_params <-
  ipw_tuning_workflow %>%
  select_by_pct_loss(metric = "roc_auc",limit=10,desc(min_n))
...
all_predictions <- bind_rows(
  augment(final_model, new_data = bmi_train) %>% 
    mutate(type = "train"),
  augment(final_model, new_data = bmi_test) %>% 
    mutate(type = "test")
) %>% 
  dplyr::select(ID,type,one_of(names(tune_grid_predictions)))

saveRDS(tune_grid_predictions,paste0(path_pasc_cmr_folder,"/working/models/pcra201_tune grid predictions from 5CV RF BMI.RDS"))
write_csv(tune_grid_metrics,"analysis/pcra201_collected metrics from 5CV RF BMI.csv")
saveRDS(final_model,paste0(path_pasc_cmr_folder,"/working/models/pcra201_best fit model from FINAL RF BMI.RDS"))

# If we were to fit it again, this is how we should do it -------
# set.seed(501)
# last_rf_fit <- best_wflow %>% 
#   fit(bmi_train)
# 
