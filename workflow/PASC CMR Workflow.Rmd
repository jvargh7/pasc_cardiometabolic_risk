...
- repo/data/pcrd02_saving data from csv to parquet.R: *All files in .csv format would be read from the source folder and saved --\> shared/working/raw/\<filename\>.parquet*
    + This file is better for storing and querying large datasets
...
This sub-step in the **PREPROCESSING** workflow consists of 4 analytic files.

- repo/preprocessing/pcrpre02_generating index date from diagnosis and lab.R: Replication code for index date
...
- repo/preprocessing/pcrpre03_identifying new-onset diabetes.R: Identifying incident T2DM cases using CPiT2DM
...
- repo/preprocessing/pcrpre05_clinical characteristics prior to infection.R: Within 1 year of index date [T0-365, T0): 
...
- repo/preprocessing/pcrpre06_healthcare utilization during followup.R: 
    + Number of outpatient visits
    + Number of telehealth encounters
    + Number of hospitalizations
    + Number of lab visits
    + Number of HbA1c measurements
- repo/data/pcrpre07_community characteristics.R
- repo/data/pcrpre08_negative control outcomes.R: Unrelated to exposure and expected to occur at all ages in the study population
    + incidence of fractures: Grouped ICD-10 codes () and procedure codes ()
    + eye infections: Grouped ICD-10 codes () and procedure codes ()


## Step 3. Clean merged datasets
...
DiagrammeR::grViz("digraph {
  ...
  node [shape = rectangle]        
  rec1 [label = 'Step 1. Analytic Dataset']
  rec2 [label = 'Step 2. Missing Data and Participants']
  rec3 [label = 'Step 3. Main analysis']
  rec3 [label =  'Step 4. Sensitivity analysis']
...
This sub-step in the **ANALYSIS** workflow consists of 4 sets of files. All files start with prefix *pcra*, followed by a number indicating file set. 

## Step 1. Analytic Dataset Creation

- repo/analysis/pcra101_creating follow-up dataset.R: This is the longitudinal dataset in **long** format after index date.
    + We set all anthropometric observations beyond the below ranges to missing
    + *Height (inches)*: [48, 90]
    + *Weight (lbs)*: [50, 500]
    + *BMI (kg/m2)*: [12, 50]
- repo/analysis/pcra102_creating lookback dataset.R: This is the cross-sectional dataset before index date.
    + We set all laboratory values beyond the below ranges to missing
    + *HbA1c (%)*: [3, 20]
- repo/analysis/pcra103_estimating individual susceptibility score: Susceptibility for T2DM prior to infection using
prediction model developed on historical control cohort
    + *This analysis is pending*


## Step 2. Accounting for Missing Data and Missing Participants

- repo/analysis/pcra201_missing data in covariates: Multiple imputation by chained equations using predictive mean matching
- repo/analysis/pcra202_ipw multiple exposure.R: Using multinomial model for estimating propensity of membership in each cohort
- repo/analysis/pcra203_ipw missing index date.R: Using logistic model for estimating propensity of having an index date
...
- repo/analysis/pcra204_estimating and assessing missing outcome.R: Using logistic model for estimating propensity of having at least one outcome observation
